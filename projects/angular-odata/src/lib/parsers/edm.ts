...
export const EDM_PARSERS: { [type: string]: Parser<any> } = {
  'Edm.Date': <Parser<Date>>{
    deserialize(value: string, options: StructuredTypeFieldOptions): Date {
      return new Date(`${value}T00:00:00.000Z`);
    },
    serialize(value: Date, options: StructuredTypeFieldOptions): string {
  ...
  'Edm.Duration': <Parser<Duration>>{
    deserialize(value: string, options: StructuredTypeFieldOptions): Duration {
    ...
    serialize(d: Duration, options: StructuredTypeFieldOptions): string {
      return [
        (d.sign === -1) ? '-' : '',
        'P',
        d.years ? d.years + 'Y' : '',
  ...
  'Edm.Decimal': <Parser<number>>{
    deserialize(value: any, options: StructuredTypeFieldOptions): number {
      if (options.ieee754Compatible) {
        return parseFloat(value);
      }
      return value;
    },
    serialize(value: number, options: StructuredTypeFieldOptions): any {
      if (options.ieee754Compatible) {
  ...
  'Edm.Binary': <Parser<ArrayBuffer>>{
    deserialize(base64: string, options: StructuredTypeFieldOptions): ArrayBuffer {
    ...
    serialize(arrayBuffer: ArrayBuffer, options: StructuredTypeFieldOptions): string {
