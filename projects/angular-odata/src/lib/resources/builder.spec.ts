import buildQuery, { Expand, OrderBy, alias, json, guid, raw, binary } from './builder';
...
describe('filter', () => {
  ...
  describe('data types', () => {
    ...
    it('should handle shorthand raw values', () => {
      const filter = { someProp: raw('2018-03-30') };
      const expected = '?$filter=someProp eq 2018-03-30';
      const actual = buildQuery({ filter });
      expect(actual).toEqual(expected);
    });
    ...
    it('should handle binary values', () => {
      const filter = {
        someProp: { eq: binary('YmluYXJ5RGF0YQ==') },
      };
    ...
    it('should handle shorthand raw values', () => {
      const filter = {
        someProp: binary('YmluYXJ5RGF0YQ=='),
      };
      const expected = "?$filter=someProp eq binary'YmluYXJ5RGF0YQ=='";
      const actual = buildQuery({ filter });
      expect(actual).toEqual(expected);
...
describe('function', () => {
  ...
  it('should support a function on a collection with a raw parameter', () => {
    const func = { Test: { SomeDate: raw('2018-08-01') } };
